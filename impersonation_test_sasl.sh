...
JDBC_URL='jdbc:hive2://localhost:10000/default;auth=maprsasl;saslQop=auth-conf;ssl=true'
...
output=$(sudo -u $SUPERUSER /opt/mapr/hive/hive-1.0/bin/beeline -u "$JDBC_URL" -f $CREATE_QUERYFILE 2>&1 )
if assert "$? -eq 0"; then
	echo ok 
else 
	echo "$output"
...
output=$(sudo -u $USER /opt/mapr/hive/hive-1.0/bin/beeline -u "$JDBC_URL" -f $CREATE_QUERYFILE2 2>&1 )
if assert "$? -eq 0"; then
	echo ok 
else 
	echo "$output"
...
output=$(sudo -u $USER /opt/mapr/hive/hive-1.0/bin/beeline -u "$JDBC_URL" -f $MAPREDUCE_QUERY2 2>&1 )
if assert "$? -eq 0"; then
	echo ok 
else 
	echo "$output"
...
output=$(sudo -u $SUPERUSER /opt/mapr/hive/hive-1.0/bin/beeline -u "$JDBC_URL" -f $MAPREDUCE_QUERY 2>&1 )
if assert "$? -eq 0"; then
	echo ok 
else 
	echo "$output"
...
output=$(sudo -u $USER /opt/mapr/hive/hive-1.0/bin/beeline -u "$JDBC_URL" -f $DROP_QUERYFILE2 2>&1 )
if assert "$? -eq 0"; then
	echo ok 
else 
	echo "$output"
...
output=$(sudo -u $USER /opt/mapr/hive/hive-1.0/bin/beeline -u "$JDBC_URL" -f $DROP_QUERYFILE 2>&1 )
if assert "$? -eq 2"; then
	echo ok 
else 
	echo "$output"
...
output=$(sudo -u $SUPERUSER /opt/mapr/hive/hive-1.0/bin/beeline -u "$JDBC_URL" -f $DROP_QUERYFILE 2>&1 )
if assert "$? -eq 0"; then
	echo ok 
else 
	echo "$output"
